...
const replaceTemplate = require('./modules/replaceTemplate'); //calculates loan and replaces HTML elements with data from JSON file
...
const server = httpServer.createServer( (req, res) =>{// call back function
    ...
    if(query.id){// if there is query parameter named id read as string
        ...
        if (pathname === '/' || pathname.toLowerCase() === '/loans') {
            ...
            const loanHTML = replaceTemplate(templateHTMLLoan, loan);// function that will calculate present value and replace the loan values in the HTML
            res.end(loanHTML);
        }
    }
    else{
